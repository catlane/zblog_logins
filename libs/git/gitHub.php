...
class SaeTOAuthV2 {
	 ...
	 * @ignore
	 */
	function accessTokenURL()  { return 'https://github.com/login/oauth/access_token'; }
	function userInfoURL()  { return 'https://api.github.com/user'; }
	/**
	 * @ignore
	 */
	function authorizeURL()    { return 'https://github.com/login/oauth/authorize'; }
...
//			$params['redirect_uri'] = $keys['redirect_uri'];
        ...
        foreach ( $token as $k => $v ) {
            ...
            if(is_array($arr) && count($arr) == 2){
                $newResult[ $arr[ 0 ] ] = $arr[ 1 ];
            }
        }
		if ( is_array($newResult) && !isset($newResult['error']) ) {
			$this->access_token = $newResult['access_token'];
			//$this->refresh_token = $token['refresh_token'];
		} else {
			echo json_encode(['error'=>0, 'msg' => '校验码验证失败，请关闭页面重新登录！' ],JSON_UNESCAPED_UNICODE);
			die;
		}
		return $newResult;
	}
